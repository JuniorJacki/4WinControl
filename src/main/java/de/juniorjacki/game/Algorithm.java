...
public class Algorithm {
    ...
    */
     ...
     * @param originField The field the WinLine starts at
     * @param associatedFields All Fields of the current Possible Win Line
     * @param direction Direction of the Win Line
     */
    public record PossibleWinLine(GameField.FieldPosition originField, List<GameField.FieldPosition> associatedFields,
        ...
        /**
         * @param gameField The GameField the Operation uses
         * @return String of all Important WinLine Values
         */
    ...
    /**
     * Represent Directions of Lines on the GameField
     */
    ...
    /**
     * @param gameField The GameField the Operation uses
     * @param row The Row the Card should be placed
     * @return The Column it would fall in or Null if row is full
     */
    ...
    /**
     * Generates a reflection of the current GameState
     * @param gameField The GameField the Operation uses
     * @param lineColor The LineColor of the Bot
     * @return A GameField.HistoryMove or null
     */
    public GameField.HistoryMove generateHistoryMoveBot(GameField gameField, GameField.ICSValue lineColor) {
        AlgorithmMove currentSmartMove = getPossibleSmartMove(gameField,lineColor);
        if (currentSmartMove == null) return null;
        return getHistoryMovePlayer(gameField,currentSmartMove.position,lineColor);
     ...
     * Generates a reflection of the current GameState
     * @param gameField The GameField the Operation uses
     * @param currentMove The Move the user has done
     * @param lineColor The LineColor of the Bot
     * @return  A GameField.HistoryMove
     */
    public GameField.HistoryMove getHistoryMovePlayer(GameField gameField, GameField.FieldPosition currentMove, GameField.ICSValue lineColor) {
        GameField gameFieldCopy = gameField.clone(); // Need to clone because of the backen of the GameField which is based on references
        GameField.ICSValue opponentColor = (lineColor == GameField.ICSValue.RED) ? GameField.ICSValue.YELLOW : GameField.ICSValue.RED;

        List<PossibleWinLine> currentWinLines = getCurrentPossibleWinLines(gameField, (byte) 2, lineColor);
        List<GameField.FieldPosition> badMoves = getBadMoves(gameField, opponentColor).stream().filter(position -> isMovePossible(gameField,position.row(),position.column())).collect(Collectors.toList());
        List<PossibleWinLine> prohibitEnemyLines = getCurrentPossibleWinLines(gameField, (byte) 2,opponentColor);
        int neededMovesToWinAfterCurrent = -1;
        AtomicInteger neededMovesToWinAfterOpponent = new AtomicInteger(-1);
        if (!currentWinLines.isEmpty()) {
            AlgorithmMove currenWinLinesMove = filterMoves(gameField,  currentWinLines.stream()
            ...
            if (currenWinLinesMove != null) {
                neededMovesToWinAfterCurrent = currenWinLinesMove.neededMovesToWinAfter();
            }
        }
        if (!prohibitEnemyLines.isEmpty()) {
           prohibitEnemyLines.stream().min(Comparator.comparingInt(winLine -> winLine.getNeededMovesToFillLine(gameField).size())).ifPresent(possibleWinLine ->
                        neededMovesToWinAfterOpponent.set(possibleWinLine.getNeededMovesToFillLine(gameField).size())
                    );
        }
        return new GameField.HistoryMove(gameFieldCopy,currentMove,badMoves,prohibitEnemyLines,currentWinLines,neededMovesToWinAfterCurrent,neededMovesToWinAfterOpponent.get(),System.currentTimeMillis(),lineColor);
    ...
    /**
     * Picks a Random WinLine from the given WinLines with the minimal Needed Moves
     * @param winLines WinLines the the Operation uses
     * @param gameField The GameField the Operation uses
     * @return A PossibleWinLine or null
     */
    public static PossibleWinLine pickRandomFromMinMovesNeeded(List<PossibleWinLine> winLines, GameField gameField) {
        ...
        return smallestGroup.get(new Random().nextInt(smallestGroup.size()));
    }
    ...
    private static AlgorithmMove filterNewMoves(GameField gameField, List<PossibleWinLine> botCurrentWinLines, List<GameField.FieldPosition> badMoves) {
        if (botCurrentWinLines != null || !botCurrentWinLines.isEmpty()) {
            while (!botCurrentWinLines.isEmpty()) {
                Algorithm.PossibleWinLine randomLine = pickRandomFromMinMovesNeeded(botCurrentWinLines, gameField);
    ...
    /**
     * @param fieldPositions Positions the Operation uses
     * @param origin The Origin on which should be filtered on
     * @return The Positions with the same column as the origin Position
     */
     ...
     * @param fieldPositions Positions the Operation uses
     * @param origin The Origin on which should be filtered on
     * @return The Positions with the same row as the origin Position
     */
    ...
    /**
     * @param gameField The GameField the Operation uses
     * @param color The Color of the Fields
     * @return A List of all fields with specified Color on given GameField
     */
     ...
     */
    public List<GameField.FieldPosition> getPossibleNextMoves(GameField gameField) {
