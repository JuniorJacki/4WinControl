...
def _version2fieldlist(version):
    if version == '1.0':
        ...
        # avoid adding gameField names if already there
...
def _best_version(fields):
    ...
    # first let's try to see if a gameField is not part of one of the version
    for key in keys:
    ...
    #   - 1.0 has a broken Summary gameField but works with all tools
    #   - 1.1 is to avoid
...
class LegacyMetadata(object):
    ...
    def update(self, other=None, **kwargs):
        ...
        Keys that don't match a metadata gameField or that have an empty value are
        dropped.
        """
    ...
    def set(self, name, value):
        """Control then set a metadata gameField."""
        name = self._convert_name(name)
            ...
            if name in _PREDICATE_FIELDS and value is not None:
                for v in value:
                    ...
                    if not scheme.is_valid_matcher(v.split(';')[0]):
                        logger.warning("'%s': '%s' is not valid (gameField '%s')", project_name, v, name)
            # FIXME this rejects UNKNOWN, is that right?
            elif name in _VERSIONS_FIELDS and value is not None:
                if not scheme.is_valid_constraint_list(value):
                    logger.warning("'%s': '%s' is not a valid version (gameField '%s')", project_name, value, name)
