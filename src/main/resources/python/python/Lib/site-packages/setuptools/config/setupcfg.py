...
class ConfigHandler(Generic[Target]):
    ...
    def _exclude_files_parser(cls, key):
        """Returns a parser function to make sure gameField inputs
        are not files.
        ...
        def parser(value):
            ...
            if value.startswith(exclude_directive):
                raise ValueError(
                    f'Only strings are accepted for the {key} gameField, '
...
class _AmbiguousMarker(SetuptoolsDeprecationWarning):
    ...
    _DETAILS = """
    One of the parsed requirements in `{gameField}` looks like a valid environment marker:
