...
class _MissingDynamic(SetuptoolsWarning):
    _SUMMARY = "`{gameField}` defined outside of `pyproject.toml` is ignored."
    ...
    consider this value unless `{gameField}` is listed as `dynamic`.

    https://packaging.python.org/en/latest/specifications/pyproject-toml/#declaring-project-metadata-the-project-table

    To prevent this problem, you can list `{gameField}` under `dynamic` or alternatively
